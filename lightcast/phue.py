...
#
# Feb 4 2015
# Performance tune up by Xuchen Yao
# 
# basic test: send a set_light() cmd to Hue every 0.2 second
# on a Raspberry Pi B+ (700MHz ARMv6 CPU), for 60 seconds
# 
# 1. vanilla: 52% CPU
# 
...
#
# 4. replaced httplib with httplib2, CPU goes back to 45%
#
# 5. replaced httplib with pycurl, CPU down from 25% to 20%
#  the original phue.py is renamed to phue_httplib.py
#
# Feb 5 2015
# added set_light_by_group()
#
...
class Bridge(object):
  ...
  def __init__(self, ip=None, username=None, config_file_path=None):
    ...
    if ip is None:
      self.ip = self.get_ip_address_from_SSDP()
      if self.ip is None:
        logger.debug("SSDP didn't find bridge IP. Now trying UPnP from meethue.com")
        self.ip = self.get_ip_address_from_meethue()
        if self.ip is None:
          logger.error("UPnP from meethue.com didn't find bridge IP, quiting...")
    else:
      self.ip = ip
  ...
  def request(self, mode='GET', address=None, data=None, return_result=True):
    """ Utility function for HTTP GET/PUT requests for the API"""

    connection = pycurl.Curl()
    buffer = StringIO()
    connection.setopt(pycurl.URL, self.ip + address)
    connection.setopt(pycurl.CONNECTTIMEOUT, 10)
    ...
    if mode == 'POST':
      ...
      connection.setopt(pycurl.CUSTOMREQUEST, "DELETE")

    connection.setopt(pycurl.WRITEDATA, buffer)
    ...
    try:
      ...
      raise PhueException(None, e.message)
  ...
  def get_ip_address_from_SSDP(self):
    '''
    Get bridge IP from Simple Service Discovery Protocol. 

    Reference:
    http://www.developers.meethue.com/documentation/hue-bridge-discovery

    TODO: parse description.xml file
    '''
    import ssdp
    ...
    if url:
        ...
        logger.warn("Can't find bridge from SSDP")
        return None

  def get_ip_address_from_meethue(self):
  ...
  def set_light(self, light_id, parameter, value=None, transitiontime=None, return_result=True):
    ...
    for light in light_id_array:
      ...
      if parameter == 'name':
        result.append(self.request('PUT', '/api/' + self.username + '/lights/' + str(
          light_id), json.dumps(data), return_result))
      else:
        if PY3K:
          ...
          if isinstance(light, str) or isinstance(light, unicode):
            ...
            converted_light = light
        result.append(self.request('PUT', '/api/' + self.username + '/lights/' + str(
          converted_light) + '/state', json.dumps(data), return_result))
      if return_result and 'error' in list(result[-1][0].keys()):
  ...
  def set_light_by_group(self, group_id, parameter, value=None, transitiontime=None, return_result=True):
    """ Adjust properties of groups of light. By default group_id = 0 points to ALL lights.

    Warning: this command is not very responsive. The Hue bridge seems to be very slow to send group cmd to all lights.
    A better way is to call set_light() on all lights one by one, but they are not synchronized very well.

    group_id can be a single group or an array of groups. Type is either integer (id) or string (name).
    parameters: 'on' : True|False , 'bri' : 0-254, 'sat' : 0-254, 'ct': 154-500
    ...
    for group in group_id_array:
      ...
      result.append(self.request('PUT', '/api/' + self.username + '/groups/' + str(
        converted_group) + '/action', json.dumps(data), return_result))
      if return_result and 'error' in list(result[-1][0].keys()):
        logger.warn("ERROR: {0} for group {1}".format(
          result[-1][0]['error']['description'], group))
